...
// Program data (inlined for server-side use)
const programs = [
  {
    id: 'bisp-kafaalat',
...
function buildProgramContext(query: string): string {
...
async function handleChatRequest(messages: ChatMessage[]): Promise<ChatResponse> {
  const apiKey = process.env.QWEN_API_KEY;
  const model = process.env.QWEN_MODEL || 'ms-f571650f-bfb0-41d6-8216-8f930f38a5ca';
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/health', (req, res) => {
        ...
        res.end(JSON.stringify({ 
          ...
          model: process.env.QWEN_MODEL || 'ms-f571650f-bfb0-41d6-8216-8f930f38a5ca'
        }));
      });
    }
  };
