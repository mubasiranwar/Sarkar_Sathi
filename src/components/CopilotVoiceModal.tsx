      ...
      <div className="relative flex min-h-[min(720px,calc(100vh-2rem))] w-full max-w-2xl flex-col overflow-hidden rounded-4xl border border-emerald-200/20 bg-[radial-gradient(circle_at_50%_20%,#123b3a_0%,#0f172a_52%,#07111b_100%)] text-white shadow-2xl shadow-emerald-950/50">
        ...
        <div className="border-t border-white/10 px-5 py-4">
          <div className="mb-3 flex flex-wrap justify-center gap-2 text-xs">
            <button onClick={onSwitchLanguage} className="inline-flex items-center gap-2 rounded-full border border-white/15 px-3 py-2 text-white/75 hover:bg-white/10" title={language === 'ur' ? 'Switch to English speech recognition' : 'اردو میں بولنے کے لیے تبدیل کریں'}><Languages className="h-3.5 w-3.5" />{language === 'ur' ? '🇵🇰 اردو میں بولیں' : '🇬🇧 Speak in English'}</button>
