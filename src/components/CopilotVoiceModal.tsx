...
export default function CopilotVoiceModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-label="Copilot Voice Mode">
      <div className="relative flex min-h-[min(720px,calc(100vh-2rem))] w-full max-w-2xl flex-col overflow-hidden rounded-4xl border border-emerald-200/20 bg-[radial-gradient(circle_at_50%_20%,#123b3a_0%,#0f172a_52%,#07111b_100%)] text-white shadow-2xl shadow-emerald-950/50">
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="font-semibold">Copilot Voice</h2><p className="text-xs text-white/55">{stateLabel}</p></div>
          </div>
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-6 py-8 text-center">
          ...
          <div className="mt-7 w-full space-y-3 text-left">
            {userTranscript && <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-white/80"><span className="mb-1 block text-[10px] uppercase tracking-[0.18em] text-emerald-200/70">You</span>{userTranscript}</div>}
        ...
        <div className="border-t border-white/10 px-5 py-4">
          ...
          <div className="flex items-center justify-center gap-3">
            <button onClick={onToggleListening} className={`flex h-14 w-14 items-center justify-center rounded-full transition ${isListening ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-emerald-300 text-slate-950 hover:bg-emerald-200'}`} aria-label={isListening ? 'Stop listening' : 'Start listening'}>{isListening ? <MicOff className="h-6 w-6" /> : <Mic className="h-6 w-6" />}</button>
            <button onClick={onSend} disabled={!userTranscript || isProcessing} className="rounded-full bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-40">Send to assistant</button>
            <button onClick={onHangUp} className="flex h-12 w-12 items-center justify-center rounded-full bg-red-500 text-white shadow-lg shadow-red-500/25 transition hover:bg-red-400" aria-label="Hang up and exit Voice Mode" title="Hang up"><PhoneOff className="h-5 w-5" /></button>
          </div>
          <p className="mt-3 text-center text-[11px] text-white/45">Speak naturally, or review the caption before sending.</p>
        </div>
      </div>
    </div>
  );
